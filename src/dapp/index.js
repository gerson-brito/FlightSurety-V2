...
(async () => {
    ...
    let contract = new Contract('localhost', () => {
        ...
        DOM.elid('check-balance').addEventListener('click', () => {
            let clientAddress = DOM.elid('client-address').value;
            contract.getClientCredit(clientAddress, (error, result) => {
                displayTx('display-wrapper-client-detail', [{ label: 'Credit pending to withdraw', error: error, value: result + ' ETH' }]);
                DOM.elid('client-address').value = "";
            });
        })

        DOM.elid('withdraw-balance').addEventListener('click', () => {
            let clientAddress = DOM.elid('client-address').value;
            contract.withdrawCredit(clientAddress, (error, result) => {
                displayTx('display-wrapper-client-detail', [{ label: 'Credit withdrawn', error: error, value: result + ' ETH' }]);
                DOM.elid('client-address').value = "";
            });
        });
    });
